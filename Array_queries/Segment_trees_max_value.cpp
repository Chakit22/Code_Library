...
void build(vector<int>&tree, int lt, int ht, int a[], int index) {
	//build function actually updates the value at index of tree as min from range lt to ht with respect to arr indices.
	if (lt > ht)
		return;  //nothing to be updated
	if (lt == ht)
	{
		tree[index] = lt;  //merges at single element present at index lt
		return;
	}
	int mid = lt + (ht - lt) / 2;
	//value at any index is min of left values and right ones.
	build(tree, lt, mid, a, 2 * index + 1);
	build(tree, mid + 1, ht, a, 2 * index + 2);
	if (a[tree[2 * index + 1]] < a[tree[2 * index + 2]])
		tree[index] = tree[2 * index + 1];
	else
		tree[index] = tree[2 * index + 2]; //current indices value stored at tree[index]  left indice is 2*index+1 and right one 2*index+2
}
...
int query_min_max(vector<int>&tree, vector<int>&lazy, int lt, int ht, int l, int h, int index) {
	if (lt > ht)
		return INT_MAX;

	if (h<lt or l>ht) //don't overlap then min will be INT_MAX
		return INT_MAX;
	...
	int mid = lt + (ht - lt) / 2;
	return min(query_min_max(tree, lazy, lt, mid, l, h, 2 * index + 1), query_min_max(tree, lazy, mid + 1, ht, l, h, 2 * index + 2)); //current ans will be min of ans from left
	//child and ans from right child.
}
...
void range_update(vector <int> &tree, vector<int>&lazy, int lt, int ht, int l, int h, int x, int index) { //update function adds the value of x to each node
	...
	if (lazy[index] != 0) {
		tree[index] += lazy[index]; //add the lazy updates to the current node
...
int main() {
	...
	int q;
	cin >> q;
	//type 1 query for range max or min
